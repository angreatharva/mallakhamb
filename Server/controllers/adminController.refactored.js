 ...
 * 
 * Handles admin HTTP endpoints by delegating to AdminService and other services.
 ...
 * - Public endpoints (scores, teams, competitions)
 * 
...
// Import models for operations not yet in services
 ...
 * @access Protected (requires authentication and competition context)
 */
const getSubmittedTeams = asyncHandler(async (req, res) => {
 ...
 * @access Protected (requires authentication and competition context)
 */
const getTeamScores = asyncHandler(async (req, res) => {
 ...
 * @access Protected (requires authentication and competition context)
 */
const getIndividualScores = asyncHandler(async (req, res) => {
  const scoringUtils = require('../utils/scoringUtils');
 ...
 * Get team rankings
 * 
 * @route GET /api/admin/scores/rankings
 * @access Protected (requires authentication and competition context)
 */
const getTeamRankings = asyncHandler(async (req, res) => {
  const { gender, ageGroup } = req.query;
  const competitionId = req.competitionId || req.query.competition;
 ...
 * @access Protected (requires authentication and competition context)
 */
const saveJudges = asyncHandler(async (req, res) => {
  ...
  try {
    // Check if judges already exist for this exact combination
    ...
    // Get all existing judges for this combination
    ...
    const emptySlots = Math.max(0, 5 - filledSlots);
...
const getJudges = asyncHandler(async (req, res) => {
  ...
  // For admin routes, only include active judges
  ...
  // Create judge slots (5 slots: 1 Senior Judge + 4 Judges)
  const judgeSlots = [
  ...
  const allJudgeSlots = judgeSlots.map(slot => {
    const judge = judges.find(j => j.judgeNo === slot.judgeNo && j.judgeType === slot.judgeType);
 ...
 * @access Protected (requires authentication and competition context)
 */
const createSingleJudge = asyncHandler(async (req, res) => {
...
const updateJudge = asyncHandler(async (req, res) => {
  ...
  // Get the judge to check their current assignment
  ...
  // Update judge fields with whitelist
 ...
 * @access Protected (requires authentication and competition context)
 */
const getAllJudgesSummary = asyncHandler(async (req, res) => {
  const competitionId = req.competitionId;
 ...
 * @access Protected (requires authentication and competition context)
 */
const startAgeGroup = asyncHandler(async (req, res) => {
  ...
  // Check if minimum judges are assigned
 ...
 * 
 * @route POST /api/scores/individual
 * @access Public
 */
const saveIndividualScore = asyncHandler(async (req, res) => {
  const scoringUtils = require('../utils/scoringUtils');
  ...
  // Convert competition type format
