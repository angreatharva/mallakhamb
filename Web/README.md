...
- **Secure Token-Based Reset**: Uses cryptographically secure tokens with SHA-256 hashing
- **Email Integration**: Automated password reset emails via Gmail SMTP
- **Time-Limited Tokens**: Reset tokens expire after 15 minutes for security
- **Universal Access**: Available for both players and coaches
- **Security Best Practices**: Prevents user enumeration and timing attacks
...
- **Nodemailer** for email services (password reset)
...
│   │   ├── playerRoutes.js
│   │   ├── coachRoutes.js
│   │   ├── authRoutes.js      # Password reset endpoints
│   │   └── teamRoutes.js
│   ├── utils/             # Utility functions
│   │   ├── emailService.js    # Email sending utility
│   │   └── tokenUtils.js      # Token generation and hashing
...
4. **Set up environment variables**
   ...
   **Gmail App Password Setup for Password Reset:**
   ...
   4. Use the generated 16-character password for `EMAIL_PASS`
   5. Never commit the `.env` file to version control
   
   For detailed setup instructions, see [Gmail Setup Guide](Server/docs/GMAIL_SETUP.md)
...
3. **Password Reset** (if needed) → Use "Forgot Password?" link to reset via email
4. **Team Selection** → Select a team to join (if not assigned)
5. **Dashboard** → View profile and team information

### Coach Flow
1. **Home Page** → Click "Are you a Coach?"
2. **Registration/Login** → Register with email or login
3. **Password Reset** (if needed) → Use "Forgot Password?" link to reset via email
4. **Create Team** → Create a team (if not created)
5. **Dashboard** → Manage team, add players to age groups
