...
describe('webVitals utilities', () => {
  ...
  it('registers web-vitals callbacks and reports received metrics', async () => {
    ...
    expect(onPerfEntry).toHaveBeenCalledWith(expect.objectContaining({ name: 'LCP', id: 'lcp-1' }));
    expect(onPerfEntry).toHaveBeenCalledWith(expect.objectContaining({ name: 'CLS', id: 'cls-1' }));
