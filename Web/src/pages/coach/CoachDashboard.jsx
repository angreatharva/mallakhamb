...
const CoachDashboard = () => {
  ...
  useEffect(() => {
    if (selectedGender && selectedPlayerData) setSelectedAgeGroup(null);
  ...
  const getAvailableAgeGroups = (gender, playerAge) => {
    ...
    for (let i = 0; i < sorted.length; i++) {
      const g = sorted[i];
      if (g.value.startsWith('Under') && playerAge <= g.maxAge) {
        idx = i;
        break;
      }
      if (g.value.startsWith('Above')) {
        idx = i;
        break;
      }
    }
  ...
  const handlePlayerSelect = (player) => {
    if (!player.dateOfBirth) {
      toast.error('Player date of birth is missing.');
      return;
    }
  ...
  const handleAddPlayer = async () => {
    ...
    if (selectedAgeGroup.value.startsWith('Under') && age > selectedAgeGroup.maxAge) {
      return toast.error(
        `Player is ${age} years old and cannot play in ${selectedAgeGroup.label} (max: ${selectedAgeGroup.maxAge})`
      );
    }
    try {
        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
          ...
          >
            ...
            <p className="text-white/45 mb-6">
              You don't have a team registered for this competition yet.
            </p>
            <button
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <span className="text-white font-semibold text-sm">
            {getAgeGroupDisplay(ageGroup)} {gender === 'Male' ? 'Boys' : 'Girls'}
          </span>
          <span
              ...
              <div className="flex items-center gap-4">
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${COLORS.saffron}28`,
                  }}
                >
                  <Trophy
                    className="w-7 h-7"
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          {[
            ...
            },
            {
              icon: Trophy,
              label: 'Age Groups',
              value: Object.keys(playersByAgeGroup).length,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <FadeIn delay={0.15}>
            ...
            >
              <h2 className="text-lg font-black text-white mb-5 flex items-center gap-2">
                <span
                  className="w-2 h-2 rounded-full bg-pink-400 inline-block"
                  aria-hidden="true"
                />
                Girls Age Groups
              </h2>
              ...
              <div className="p-6 space-y-6">
                ...
                >
                  <p className="text-white/70 text-sm">
                    <span className="text-white font-semibold">Team:</span> {team.name}
                  </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    ...
                    {
                      label: 'Girls',
                      values: femaleAgeGroupValues,
                      gender: 'Female',
                      color: '#EC4899',
                    },
                  ].map(({ label, values, gender, color }) => (
                    <div key={gender}>
                      <h4 className="font-bold text-sm mb-3" style={{ color }}>
                        {label} Age Groups
                      </h4>
                        ...
                        {values.map((ag) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {gPlayers.length > 0 ? (
                                gPlayers.map((p, i) => (
                                  <p key={i} className="text-white/55 text-xs">
                ...
                <div
                  className="flex flex-col gap-3 sm:flex-row sm:justify-end pt-4 border-t"
                ...
                >
                  ...
                  {team?.isSubmitted ? (
                    <div
                      className="px-6 py-3 rounded-xl text-center min-h-[44px] flex items-center justify-center"
                      style={{
      ...
      <AnimatePresence>
        {showAddPlayer && (
          ...
          >
            ...
            />
            <motion.div
              className="relative w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl border"
              style={{ background: COLORS.darkElevated, borderColor: COLORS.darkBorderSubtle }}
              initial={{ scale: 0.95, opacity: 0, y: 20 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.95, opacity: 0, y: 20 }}
                      ...
                      {players.map((player, i) => (
                        ...
                        >
                          ...
                          <p className="text-white/40 text-xs">
                            {player.email} · {player.gender} · Age:{' '}
                            {player.dateOfBirth ? calculateAge(player.dateOfBirth) : 'Unknown'}
                          </p>
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      <p className="text-white/45 text-xs mt-1">
                        Gender: {selectedPlayerData.gender} · Age: {selectedPlayerData.age} yrs ·
                    ...
                    <div>
                      ...
                      <div
                        className="px-4 py-3 rounded-xl text-white/50 min-h-[44px] flex items-center"
                        style={{
                          background: 'rgba(255,255,255,0.03)',
                          border: `1px solid ${COLORS.darkBorderSubtle}`,
                        }}
                      ...
                      <p className="text-white/25 text-xs mt-1">
                        Automatically set from player's registration
                      </p>
                    </div>
                    <div>
                      <label
                      ...
                      >
                        Age Group{' '}
                        <span aria-hidden="true" style={{ color: COLORS.saffron }}>
                          *
                        </span>
                      </label>
                      <Dropdown
                        options={
                          selectedGender && selectedPlayerData
                            ? getAvailableAgeGroups(selectedGender.value, selectedPlayerData.age)
                            : []
                        }
