  ...
  const handlePayment = async () => {
    try {
      ...
      if (!isLoaded) {
        setProcessing(false);
        toast.error(
          'Razorpay checkout failed to load. Disable ad-block/privacy extensions or try another network.'
        );
        return;
      }
        ...
        <FadeIn>
          ...
          <div className="mb-8">
            ...
            <p className="text-white/40 text-sm mt-1">
              Complete your team registration for the competition
            </p>
          </div>
        </FadeIn>
              ...
              >
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-white/50">
                    Per Player ({team?.players?.length || 0} × ₹100)
                  </span>
                  <span className="text-white">
                    ₹{((team?.players?.length || 0) * 100).toLocaleString()}
                  </span>
                </div>
                <div
