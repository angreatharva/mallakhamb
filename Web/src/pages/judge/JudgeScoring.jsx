...
import { Scale, Users, User, Trophy, LogOut, CheckCircle, RotateCcw, Send } from 'lucide-react';
  ...
  >
    ...
    >
      ...
      <span
        ...
        style={{ color: active ? '#fff' : done ? J.green : 'rgba(255,255,255,0.4)' }}
      >
        {title}
      </span>
    </div>
...
const CheckRow = ({ checked, onChange, label }) => (
  <label
    className="flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer transition-all duration-200 min-h-[52px]"
    style={{
  ...
  >
    <div
      ...
      style={{
        borderColor: checked ? J.green : 'rgba(255,255,255,0.2)',
...
const JudgeScoring = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setJudgeInfo(judge);
      if (judge.competitionTypes?.length === 1)
  ...
  const handleTeamSelect = (team) => {
    ...
    const filtered = team.players
      .filter(
        (e) => e.player && e.player.gender === judgeInfo.gender && e.ageGroup === judgeInfo.ageGroup
      )
      .map((e) => ({
        id: e.player._id,
  ...
  const calcDiffTotal = (a, b, c) => a * 0.2 + b * 0.4 + c * 0.6;
  ...
  const handleCombination = (field) => {
    ...
    let deductions = 0;
    if (!c.fullApparatusUtilization) deductions += 0.4;
    if (!c.rightLeftExecution) deductions += 0.4;
    if (!c.forwardBackwardFlexibility) deductions += 0.4;
    if (!c.minimumElementCount) deductions += 0.4;
    c.total = Math.max(0, 1.6 - deductions);
    ...
    try {
      ...
      if (socket?.connected) {
        socket.emit('score_update', {
          ...scoreData,
          roomId: `scoring_${judgeInfo.gender}_${judgeInfo.ageGroup}_${selectedCompetitionType}`,
        });
      }
        ...
        <div className="max-w-2xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs truncate" style={{ color: J.purpleLight }}>
                {judgeInfo.judgeType}
              </p>
            </div>
          </div>
          ...
          <SectionCard step={1} title="Select Competition Type" active={step === 1} done={step > 1}>
            <div className="space-y-2">
              {competitionTypes
                .filter(
                  (ct) =>
                    !judgeInfo.competitionTypes || judgeInfo.competitionTypes.includes(ct.value)
                )
                .map((ct) => (
                  <SelectBtn
                    key={ct.value}
                    selected={selectedCompetitionType === ct.value}
                    onClick={() => {
                      setSelectedCompetitionType(ct.value);
                      setSelectedTeam(null);
                      setSelectedPlayer(null);
                      resetScores();
                    }}
          ...
          <SectionCard step={3} title="Select Player" active={step === 3} done={step > 3}>
            {players.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                />
                <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>
                  No players found in this team for selected category
                </p>
              </div>
            )}
          </SectionCard>
            ...
            >
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span
                    ...
                    style={{ background: 'rgba(168,85,247,0.15)', color: J.purpleLight }}
                  >
                    Max 3.80
                  </span>
                </div>
                <div
                  ...
                  style={{ background: 'rgba(168,85,247,0.08)', color: 'rgba(255,255,255,0.5)' }}
                >
                ...
                <div className="flex justify-end">
                  <span
                    ...
                    style={{
                      color: Math.abs(scores.difficulty.total - 3.8) < 0.01 ? J.green : J.amber,
                    }}
                  >
                    {scores.difficulty.total.toFixed(2)}{' '}
                    <span
              ...
              >
                ...
                {[
                  ...
                  <CheckRow
                    key={field}
                    checked={scores.combination[field]}
                    onChange={() => handleCombination(field)}
                    label={label}
                  />
                ))}
                <div className="flex justify-end pt-1">
                  <span className="text-lg font-black" style={{ color: J.purple }}>
                    {scores.combination.total.toFixed(2)}{' '}
                    <span
                      className="text-xs font-normal"
                      style={{ color: 'rgba(255,255,255,0.3)' }}
                    >
              ...
              >
                ...
                <ScoreInput
                  ...
                  step={0.1}
                  onChange={(e) => {
                    const v = Math.max(0, Math.min(4.4, parseFloat(e.target.value) || 0));
                    setScores({ ...scores, execution: v });
                  }}
                />
              </div>
              ...
              >
                ...
                <ScoreInput
                  ...
                  onChange={(e) => {
                    ...
                    setScores({ ...scores, originality: v });
                  }}
                />
              </div>
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-4 gap-2 relative z-10">
                  ...
                  <ScoreRow
                    label="Execution"
                    value={parseFloat(scores.execution || 0)}
                    max="4.40"
                  />
                  <ScoreRow
                    label="Originality"
                    value={parseFloat(scores.originality || 0)}
                    max="0.20"
                  />
                </div>
              </motion.div>
              ...
              <motion.button
                ...
                style={{
                  background: submitted
                    ? `linear-gradient(135deg, ${J.green}, ${J.greenDark})`
                    : `linear-gradient(135deg, ${J.purple}, ${J.purpleDark})`,
                  opacity: !isConnected || submitting ? 0.6 : 1,
                }}
                whileHover={
                  reduced || submitting || !isConnected
                    ? {}
