...
const DarkInput = ({ label, required, error, hint, ...props }) => (
  <div>
    {label && (
      ...
      >
        {label}
        {required && <span style={{ color: ADMIN_COLORS.red }}> *</span>}
      </label>
    )}
    <input
      ...
      style={{
        ...
        border: `1px solid ${error ? ADMIN_COLORS.red + '60' : ADMIN_COLORS.darkBorderMid}`,
      ...
      onBlur={(e) => {
        e.target.style.borderColor = error ? ADMIN_COLORS.red + '60' : ADMIN_COLORS.darkBorderMid;
    ...
    {error && (
      <p className="text-xs mt-1" style={{ color: ADMIN_COLORS.red }}>
        {error}
      </p>
    )}
  </div>
...
const DarkBtn = ({
  ...
  return (
    <motion.button
      ...
      style={{
        background: bg,
        border: variant === 'ghost' ? `1px solid ${ADMIN_COLORS.darkBorderSubtle}` : 'none',
        minHeight: h,
        padding: size === 'sm' ? '0 0.75rem' : '0 1.25rem',
      }}
...
const StatusBadge = ({ active }) => (
  <span
    ...
    style={{
      background: active ? `${ADMIN_COLORS.green}20` : `${ADMIN_COLORS.red}20`,
      color: active ? ADMIN_COLORS.green : ADMIN_COLORS.red,
    }}
  >
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            className={`w-full ${isMobile ? 'max-w-full' : maxWidth} max-h-[90vh] flex flex-col rounded-3xl border overflow-hidden`}
            style={{
          ...
          >
            ...
            {footer && (
              <div
                className="flex gap-3 border-t flex-shrink-0"
                style={{
                  borderColor: ADMIN_COLORS.darkBorderSubtle,
                  padding: isMobile ? '1rem' : '1.5rem',
...
const SuperAdminManagement = () => {
  ...
  const [competitionFormData, setCompetitionFormData] = useState({
    name: '',
    level: { value: 'district', label: 'District' },
  ...
  const ageGroupLabels = {
    ...
    Under12: 'Under 12',
    Under14: 'Under 14',
    Under16: 'Under 16 (Girls)',
    Under18: 'Under 18 (Boys)',
    Above16: 'Above 16 (Girls)',
    Above18: 'Above 18 (Boys)',
  };
  ...
  const openCompetitionModal = (comp = null) => {
    if (comp) {
      ...
      setCompetitionFormData({
        ...
        admins: comp.admins.map((a) => a._id),
  ...
  const handleCreateCompetition = async (e) => {
    e.preventDefault();
    if (!competitionFormData.admins.length) {
      toast.error('Assign at least one admin');
      return;
    }
    if (!competitionFormData.ageGroups.length) {
      toast.error('Select at least one age group');
      return;
    }
    if (!competitionFormData.competitionTypes.length) {
      toast.error('Select at least one competition type');
      return;
    }
    try {
  ...
  const toggleCompetitionType = (type) =>
    setCompetitionFormData((prev) => ({
      ...prev,
      competitionTypes: prev.competitionTypes.includes(type)
        ? prev.competitionTypes.filter((t) => t !== type)
        : [...prev.competitionTypes, type],
    }));

  const toggleAgeGroup = (ag, gender) =>
    setCompetitionFormData((prev) => {
      const groups = [...prev.ageGroups];
      const idx = groups.findIndex((g) => g.ageGroup === ag && g.gender === gender);
      if (idx >= 0) groups.splice(idx, 1);
      else groups.push({ ageGroup: ag, gender });
      return { ...prev, ageGroups: groups };
    });

  const isAgeGroupSelected = (ag, gender) =>
    competitionFormData.ageGroups.some((g) => g.ageGroup === ag && g.gender === gender);
  const isAgeGroupValidForGender = (ag, gender) =>
    gender === 'Male'
  ...
  const toggleAdminSelection = (id) =>
    setCompetitionFormData((prev) => ({
      ...prev,
      admins: prev.admins.includes(id) ? prev.admins.filter((a) => a !== id) : [...prev.admins, id],
    }));

  // ─── Admin management modal ────────────────────────────────────────────────
  const openAdminManagementModal = (comp) => {
  ...
  const levelColor = (l) =>
    ({
      district: ADMIN_COLORS.blue,
      state: ADMIN_COLORS.purple,
      national: ADMIN_COLORS.saffron,
      international: ADMIN_COLORS.gold,
    })[l] || ADMIN_COLORS.saffron;
  const statusColor = (s) =>
    ({
      ongoing: ADMIN_COLORS.green,
      upcoming: ADMIN_COLORS.gold,
          ...
          <DarkCard className="p-6">
            ...
            {loading ? (
              ...
              <EmptyState
                ...
                desc="Create your first competition to get started."
              />
            ) : (
              <DarkTable
                headers={[
                  ...
                  <DarkTr key={comp._id} delay={i * 0.03}>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {(comp.competitionTypes || []).map((t) => (
                          <span
                            ...
                            style={{
                              background: `${ADMIN_COLORS.purple}20`,
                              color: ADMIN_COLORS.purpleLight,
                            }}
                          >
                            {compTypeLabel(t)}
                          </span>
                    ...
                    <Td>
                      <span
                        ...
                        style={{
                          background: `${levelColor(comp.level)}20`,
                    ...
                    <Td>
                      <span
                        className="px-2 py-0.5 rounded-full text-xs font-bold capitalize"
                        style={{
                          background: `${statusColor(comp.status)}20`,
                    ...
                    <Td>
                      <div className="space-y-0.5">
                        {comp.admins?.length ? (
                          comp.admins.map((a) => (
                            <p key={a._id} className="text-xs text-white/60">
                              {a.name}
                            </p>
                          ))
                        ) : (
                    ...
                    <Td>
                      <span
                        ...
                        style={{
                          background:
                            admin.role === 'super_admin'
                              ? `${ADMIN_COLORS.saffron}20`
                              : `${ADMIN_COLORS.blue}20`,
                          color:
                            admin.role === 'super_admin' ? ADMIN_COLORS.saffron : ADMIN_COLORS.blue,
                        }}
                      >
              ...
              <DarkTable headers={['Name', 'Email', 'Phone', 'Status', 'Actions']}>
                {coaches.map((coach, i) => (
                  <DarkTr key={coach._id} delay={i * 0.03}>
                    ...
                    <Td>{coach.phone || '—'}</Td>
                    <Td>
                      <StatusBadge active={coach.isActive} />
                    </Td>
                    <td className="px-4 py-3">
                      <DarkBtn
                        size="sm"
            ...
            <form onSubmit={handleAddPlayer} className="max-w-2xl space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <DarkInput
                  ...
                  onChange={(e) =>
                    setPlayerFormData({ ...playerFormData, password: e.target.value })
                  }
                />
                <DarkInput
                  label="Phone"
                ...
                <DarkInput
                  ...
                  onChange={(e) =>
                    setPlayerFormData({ ...playerFormData, dateOfBirth: e.target.value })
                  }
                />
                <Dropdown
                  label="Gender"
                ...
                <Dropdown
                  ...
                  onChange={(option) =>
                    setPlayerFormData({ ...playerFormData, competitionId: option, teamId: null })
                  }
                  ...
                  options={teams
                    .filter(
                      (t) =>
                        ...
                        (t.competition?._id === playerFormData.competitionId.value ||
                          t.competitionId === playerFormData.competitionId.value)
                    )
                    .map((t) => ({
                      value: t._id,
                ...
                <Dropdown
                  ...
                  onChange={(option) =>
                    setPlayerFormData({ ...playerFormData, paymentStatus: option })
                  }
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <div className="relative">
                ...
                <div
                  ...
                  style={{
                    background: formData.isActive ? ADMIN_COLORS.green : 'rgba(255,255,255,0.15)',
                  }}
                >
                  <div
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <DarkInput
            ...
            onChange={(e) =>
              setCompetitionFormData({ ...competitionFormData, name: e.target.value })
            }
          />
          <Dropdown
            label="Level"
          ...
          <DarkInput
            ...
            onChange={(e) =>
              setCompetitionFormData({ ...competitionFormData, place: e.target.value })
            }
          />
          <Dropdown
            label="Year"
          ...
          <DarkInput
            ...
            onChange={(e) =>
              setCompetitionFormData({ ...competitionFormData, startDate: e.target.value })
            }
          />
          <DarkInput
            label="End Date"
            ...
            onChange={(e) =>
              setCompetitionFormData({ ...competitionFormData, endDate: e.target.value })
            }
          />
        </div>
            ...
            {[
              {
                id: 'competition_1',
                label: 'Competition I',
                desc: 'Team Championship & Qualifier',
              },
              ...
              return (
                <label
                  ...
                  style={{
                    background: checked ? `${ADMIN_COLORS.purple}10` : 'rgba(255,255,255,0.02)',
                    borderColor: checked
                      ? `${ADMIN_COLORS.purple}40`
                      : ADMIN_COLORS.darkBorderSubtle,
                  }}
                >
                  <input
                    type="checkbox"
                    className="sr-only"
                    checked={checked}
                    onChange={() => toggleCompetitionType(ct.id)}
                  />
                  <div
                    className="w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 mt-0.5 transition-all"
                    style={{
                      background: checked ? ADMIN_COLORS.purple : 'transparent',
                      borderColor: checked ? ADMIN_COLORS.purple : 'rgba(255,255,255,0.3)',
                    }}
                  >
                    {checked && (
                      <svg
        ...
        <DarkTextarea
          ...
          onChange={(e) =>
            setCompetitionFormData({ ...competitionFormData, description: e.target.value })
          }
        />
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['Male', 'Female'].map((gender) => {
                    ...
                    return (
                      <label
                        key={gender}
                        className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-all ${!valid ? 'opacity-30 cursor-not-allowed' : ''}`}
                        style={{
                          background:
                            checked && valid ? `${ADMIN_COLORS.saffron}12` : 'transparent',
                        }}
                      >
                        <input
                          type="checkbox"
                          ...
                          checked={checked}
                          disabled={!valid}
                          onChange={() => valid && toggleAgeGroup(ag, gender)}
                        />
                        <div
                          className="w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 transition-all"
                          style={{
                            background: checked && valid ? ADMIN_COLORS.saffron : 'transparent',
                            borderColor:
                              checked && valid ? ADMIN_COLORS.saffron : 'rgba(255,255,255,0.3)',
                          }}
                        >
                          {checked && valid && (
                            <svg
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className="w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 transition-all"
                      style={{
                        background: checked ? ADMIN_COLORS.saffron : 'transparent',
                        borderColor: checked ? ADMIN_COLORS.saffron : 'rgba(255,255,255,0.3)',
                      }}
                    >
                      {checked && (
                        <svg
                    ...
                    <span className="text-sm text-white/70">
                      {admin.name} <span className="text-white/35">({admin.email})</span>
                    </span>
                  </label>
                );
              })}
              {filteredAdmins.length === 0 && (
                <p className="text-sm text-white/30 p-2">No admins found.</p>
              )}
            </div>
          </div>
        )}
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {admins
                  .filter(
                    (a) =>
                      a.role !== 'super_admin' &&
                      !selectedCompetition.admins?.find((sa) => sa._id === a._id)
                  )
                  .map((admin) => (
                    <div
                      key={admin._id}
