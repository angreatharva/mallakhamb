...
const RankBadge = ({ rank }) => {
  ...
  if (c) {
    return (
      <div
        className="w-8 h-8 rounded-full flex items-center justify-center border text-sm font-black flex-shrink-0"
...
const SkeletonRow = () => (
  <div
    className="flex items-center gap-4 p-4 rounded-2xl border animate-pulse"
    ...
    >
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        <div className="text-right flex-shrink-0">
          <p className="text-2xl font-black" style={{ color: COLORS.saffron }}>
      ...
      <div className="px-4 pb-4 flex flex-wrap gap-2">
        ...
        <ScorePill
          label="Avg"
          value={player.averageMarks.toFixed(2)}
          accent={COLORS.saffronLight}
        />
        {player.deduction > 0 && (
          <ScorePill label="Deduct" value={`-${player.deduction}`} accent="#EF4444" />
        )}
        {player.otherDeduction > 0 && (
          <ScorePill label="Other" value={`-${player.otherDeduction}`} accent="#EF4444" />
        )}
        {player.baseScoreApplied && (
          <span
            className="text-xs px-2 py-1 rounded-xl font-bold self-center"
            style={{
          ...
          {expanded && (
            ...
            >
              ...
              {(scoreEntry.timeKeeper || scoreEntry.scorer || scoreEntry.remarks) && (
                <div
                  className="px-5 pb-4 pt-2 border-t flex flex-wrap gap-4 text-xs"
                  style={{ borderColor: 'rgba(255,255,255,0.05)' }}
                >
                  {scoreEntry.timeKeeper && (
                    <span style={{ color: 'rgba(255,255,255,0.35)' }}>
                      <span className="font-bold text-white/60">Timekeeper:</span>{' '}
                      {scoreEntry.timeKeeper}
                    </span>
                  )}
                  {scoreEntry.scorer && (
...
const PublicScores = () => {
  ...
  const getFilteredScores = () => {
    ...
    return scores
      .map((entry) => ({
        ...entry,
        playerScores: entry.playerScores.filter(
          (p) => !searchTerm || p.playerName?.toLowerCase().includes(searchTerm.toLowerCase())
        ),
      }))
  ...
  return (
    ...
    >
      ...
      <main
        className="relative z-10 max-w-5xl mx-auto px-4 md:px-8 py-8 space-y-6"
        id="main-content"
      >
        {/* Page title */}
        <FadeIn>
