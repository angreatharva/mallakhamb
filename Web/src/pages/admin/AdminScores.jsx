...
} from '../../components/responsive/ResponsiveRankings';
...
const Scores = () => {
  ...
  const getFilteredData = () => {
    ...
    return scores
      .map((score, i) => ({ ...score, originalRank: i + 1 }))
      .filter((score) => {
        if (!searchTerm) return true;
        const s = searchTerm.toLowerCase();
        if (scoreType === 'individual')
          return (
            (score.playerName || '').toLowerCase().includes(s) ||
            (score.teamName || '').toLowerCase().includes(s)
          );
        ...
        <DarkCard className="p-6">
          ...
          {!filtersReady ? (
            <EmptyState
              ...
              desc="Choose a competition above to start filtering scores."
            />
          ) : !selectedGender || !selectedAgeGroup || !selectedCompetitionType ? (
            <EmptyState
              icon={Filter}
              title="Select Filters to View Scores"
              desc="Select gender, age group, and competition type to load data."
            />
          ) : loading ? (
                ...
                <div>
                    ...
                    <>
                      <InfoBanner color={ADMIN_COLORS.green}>
                        ...
                        <span className="text-white/50 ml-2">
                          ({submittedTeams.length} team{submittedTeams.length !== 1 ? 's' : ''}
                  ...
                  <div className="space-y-4">
                    {submittedTeams.length > 0 ? (
                      ...
                      <EmptyState
                        icon={Trophy}
                        title="No teams submitted yet"
                        desc="Teams appear here after coaches complete registration and payment."
                      />
                    )}
                  </div>
                </div>
              ...
              {scoreType === 'individual' && (
                <div>
                  ...
                  {scores.length > 0 ? (
                    <>
                      ...
                      {getFilteredData().length === 0 && searchTerm ? (
                        <EmptyState
                          ...
                          desc={`No players match "${searchTerm}"`}
                          action={
                            <button
                              onClick={() => setSearchTerm('')}
                              className="text-sm font-semibold"
                    ...
                    <EmptyState
                      icon={Trophy}
                      title="No individual scores found"
                      desc="Scores appear here after judges complete scoring."
                    />
                  )}
                </div>
              )}
                  ...
                  {scores.length > 0 ? (
                    <>
                      <InfoBanner color={ADMIN_COLORS.purple}>
                        ...
                        <span className="text-white/50 ml-2">
                          ({scores.length} team{scores.length !== 1 ? 's' : ''}
                          {searchTerm && `, ${getFilteredData().length} after search`})
                        </span>
                      </InfoBanner>
                      <div className="mb-4">
                    ...
                    <EmptyState
                      icon={Trophy}
                      title="No team rankings found"
                      desc="Rankings appear here after teams complete scoring."
                    />
                  )}
                </div>
              )}
