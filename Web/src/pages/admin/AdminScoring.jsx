...
const AdminScoring = () => {
  ...
  const { selectedTeam, selectedGender, selectedAgeGroup, selectedCompetitionType } =
    location.state || {};
  ...
  useEffect(() => {
    ...
    const handleScoreUpdate = (data) => {
      if (isLocked) {
        toast.info(`Score update received but locked`);
        return;
      }
      ...
      setScores((prev) => ({
        ...prev,
        [data.playerId]: { ...prev[data.playerId], [fieldName]: data.score.toString() },
      }));
  ...
  const fetchScoringData = async () => {
    try {
      ...
      const activeJudges = judgesResponse.data.judges
        .filter((j) => !j.isEmpty && j.name?.trim())
      ...
      const teamPlayers = selectedTeam.players
        .filter(
          (pe) =>
            pe.player.gender === selectedGender.value && pe.ageGroup === selectedAgeGroup.value
        )
        .map((pe) => ({
          id: pe.player._id,
          name: `${pe.player.firstName} ${pe.player.lastName}`,
          gender: pe.player.gender,
  ...
  const loadExistingScores = async (teamId, gender, ageGroup, initialScores, currentPlayers) => {
    try {
      ...
      existingScore.playerScores.forEach((ps) => {
        let targetId =
          ps.playerId && existingScores[ps.playerId]
            ? ps.playerId
            : currentPlayers.find((p) => p.name === ps.playerName)?.id;
        if (targetId && existingScores[targetId]) {
          existingScores[targetId] = {
            time: ps.time || '',
            seniorJudge: ps.judgeScores?.seniorJudge?.toString() || '',
            judge1: ps.judgeScores?.judge1?.toString() || '',
            judge2: ps.judgeScores?.judge2?.toString() || '',
            judge3: ps.judgeScores?.judge3?.toString() || '',
            judge4: ps.judgeScores?.judge4?.toString() || '',
            deduction: ps.deduction?.toString() || '',
            otherDeduction: ps.otherDeduction?.toString() || '',
          };
        }
      });
      ...
      toast.success(
        existingScore.isLocked ? 'Found locked scoring session' : 'Found existing scoring session'
      );
    } catch (error) {
  ...
  const calculateAverageMarks = (playerScores) => {
    ...
    if (executionScores.length === 0)
      return { average: 0, baseScoreApplied: false, executionAvg: 0, baseScore: 0 };
    let executionAverage = 0;
    if (executionScores.length <= 2)
      executionAverage = executionScores.reduce((s, v) => s + v, 0) / executionScores.length;
    else if (executionScores.length === 3)
      executionAverage = [...executionScores].sort((a, b) => a - b)[1];
    else {
      const sorted = [...executionScores].sort((a, b) => a - b);
      const trimmed = sorted.slice(1, -1);
      executionAverage = trimmed.reduce((s, v) => s + v, 0) / trimmed.length;
    }
    ...
    if (Math.abs(executionAverage - seniorJudge) <= tolerance)
      return {
        average: parseFloat(executionAverage.toFixed(2)),
        baseScoreApplied: false,
        executionAvg: parseFloat(executionAverage.toFixed(2)),
  ...
  const getTolerance = (score) => {
    ...
    if (score >= 7) return 0.3;
    if (score >= 6) return 0.4;
    if (score >= 5) return 0.5;
    return 1.0;
  };
  const calculateFinalScore = (ps) => {
    const r = calculateAverageMarks(ps);
    return Math.max(
      0,
      r.average - (parseFloat(ps.deduction) || 0) - (parseFloat(ps.otherDeduction) || 0)
    ).toFixed(2);
  };
  ...
  const handleSaveScores = async () => {
    ...
    try {
      const hasAnyScores = players.some((p) => {
        const ps = scores[p.id];
        return ps && (ps.seniorJudge || ps.judge1 || ps.judge2 || ps.judge3 || ps.judge4);
      });
      if (!hasAnyScores) {
      ...
      const allPlayersScored = players.every((p) => {
        ...
        if (!ps) return false;
        const hasJudge =
          (ps.seniorJudge && parseFloat(ps.seniorJudge) > 0) ||
          (ps.judge1 && parseFloat(ps.judge1) > 0) ||
          (ps.judge2 && parseFloat(ps.judge2) > 0) ||
          (ps.judge3 && parseFloat(ps.judge3) > 0) ||
          (ps.judge4 && parseFloat(ps.judge4) > 0);
        return hasJudge && ps.time?.trim();
      });
      ...
      const scoringData = {
        ...
        playerScores: players.map((p) => {
          const r = calculateAverageMarks(scores[p.id]);
          return {
            playerId: p.id,
            playerName: p.name,
            time: scores[p.id].time,
            judgeScores: {
              seniorJudge: parseFloat(scores[p.id].seniorJudge) || 0,
              judge1: parseFloat(scores[p.id].judge1) || 0,
              judge2: parseFloat(scores[p.id].judge2) || 0,
              judge3: parseFloat(scores[p.id].judge3) || 0,
              judge4: parseFloat(scores[p.id].judge4) || 0,
            },
            executionAverage: r.executionAvg,
            baseScore: r.baseScore,
            baseScoreApplied: r.baseScoreApplied,
            toleranceUsed: r.tolerance,
            averageMarks: r.average,
            deduction: parseFloat(scores[p.id].deduction) || 0,
            otherDeduction: parseFloat(scores[p.id].otherDeduction) || 0,
            finalScore: parseFloat(calculateFinalScore(scores[p.id])),
          };
        }),
        judgeDetails: judges.map((j) => ({
          judgeId: j._id,
          judgeName: j.name,
      ...
      toast.success(shouldLock ? '🎉 Scores saved and locked!' : 'Scores saved as draft.', {
        duration: 3000,
      });
      if (
        socket &&
        ...
        socket.emit('scores_saved', {
          teamId: selectedTeam._id,
          roomId: `scoring_${selectedGender.value}_${selectedAgeGroup.value}_${selectedCompetitionType.value}`,
          isLocked: shouldLock,
        });
      }
    } catch (error) {
  ...
  const handleUnlockScores = async () => {
    if (ageGroupStarted) {
      toast.error('Cannot unlock. Age group has been started.');
      return;
    }
    if (!existingScoreId) {
    ...
    return (
      ...
      >
        <div className="text-center">
          <div
            className="w-10 h-10 border-2 border-white/10 rounded-full mx-auto mb-4 animate-spin"
      ...
      >
        <div className="text-center">
          <p className="text-white/50 mb-4">
            Invalid scoring session. Please go back and select a team.
          </p>
          <motion.button
            onClick={() => navigate(`${routePrefix}/dashboard/scores`)}
            className="px-5 py-2.5 rounded-xl text-sm font-bold text-white min-h-[44px]"
            style={{
              background: `linear-gradient(135deg, ${ADMIN_COLORS.purple}, ${ADMIN_COLORS.purpleDark})`,
            }}
  ...
  return (
    <div
      className="min-h-dvh"
      style={{ background: ADMIN_COLORS.dark, fontFamily: "'Inter', system-ui, sans-serif" }}
    >
      <motion.header
      ...
      >
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between h-16">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-white font-bold text-sm hidden sm:inline">
                {routePrefix === '/superadmin' ? 'Super Admin' : 'Admin'} Scoring
              </span>
          ...
          <span
            ...
            style={
              isLocked
                ? { background: `${ADMIN_COLORS.red}20`, color: ADMIN_COLORS.red }
                : { background: `${ADMIN_COLORS.green}20`, color: ADMIN_COLORS.green }
            }
          >
            {isLocked ? (
              <>
              ...
              <>
                <div
                  ...
                  style={{ background: isConnected ? ADMIN_COLORS.green : ADMIN_COLORS.red }}
                />
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs mt-0.5 text-white/50">
                  {ageGroupStarted
                    ? 'Age group started. Scores cannot be modified.'
                    : 'Scores saved and locked. Click Unlock to edit.'}
                </p>
              </div>
            </div>
            {!ageGroupStarted && (
              <motion.button
                ...
                style={{
                  background: `${ADMIN_COLORS.red}40`,
                  border: `1px solid ${ADMIN_COLORS.red}50`,
                }}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.97 }}
              >
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
            <div>
              ...
              <div className="mt-2 space-y-1 text-sm text-white/50">
                ...
                <p>
                  Coach: <span className="text-white/70">{selectedTeam.coach?.name}</span>
                </p>
                {selectedCompetitionType && (
                  <p>
                    Competition:{' '}
                    <span className="text-white/70">{selectedCompetitionType.label}</span>
                  </p>
                )}
                {lastUpdated && (
                  <p>
                    Last updated:{' '}
                    <span className="text-white/40">{lastUpdated.toLocaleString()}</span>
                  </p>
                )}
              </div>
            </div>
            <div className="text-right flex-shrink-0">
              ...
              <p className="text-4xl font-black" style={{ color: ADMIN_COLORS.saffron }}>
                {players.length}
              </p>
            </div>
          </div>
          <div
            ...
            style={{
              background: `${ADMIN_COLORS.purple}10`,
              borderColor: `${ADMIN_COLORS.purple}30`,
            }}
          >
            <p
              className="text-xs font-bold tracking-widest uppercase mb-1"
              style={{ color: ADMIN_COLORS.purpleLight }}
            >
              Competition Type
            </p>
          ...
          <div>
            ...
            {judges.length > 0 ? (
                ...
                {judges.map((judge) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background: isSenior
                          ? `${ADMIN_COLORS.purple}12`
                          : `${ADMIN_COLORS.blue}10`,
                        borderColor: isSenior
                          ? `${ADMIN_COLORS.purple}30`
                          : `${ADMIN_COLORS.blue}25`,
                      }}
                    >
                      <span
                        ...
                        style={{
                          background: isSenior
                            ? `${ADMIN_COLORS.purple}25`
                            : `${ADMIN_COLORS.blue}25`,
                          color: isSenior ? ADMIN_COLORS.purpleLight : '#93C5FD',
                        }}
                      >
                        {judge.judgeType}
                      </span>
              ...
              <p className="text-white/30 text-sm text-center py-4">
                No judges found. Please add judges first.
              </p>
            )}
          </div>
        </div>
        ...
        <div
          ...
          style={{ background: `${ADMIN_COLORS.blue}08`, borderColor: `${ADMIN_COLORS.blue}25` }}
        >
          <p
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-white/50">
            <p>
              <span className="text-white/70 font-semibold">Execution Average:</span> From J1–J4.
              With 4 judges, highest and lowest excluded.
            </p>
            <p>
              <span className="text-white/70 font-semibold">Base Score:</span> If diff exceeds
              tolerance, Base = (Exec + Senior) / 2
            </p>
            <p>
              <span className="text-white/70 font-semibold">Tolerance:</span> 9.0+: ±0.10 · 8.0:
              ±0.20 · 7.0: ±0.30 · 6.0: ±0.40 · 5.0: ±0.50 · Below 5: ±1.00
            </p>
            <p>
              <span className="text-white/70 font-semibold">Final Score:</span> Avg Marks − Time
        ...
        {players.length > 0 ? (
          ...
          >
            <p
              ...
              style={{ color: ADMIN_COLORS.saffron }}
            >
              <Clock className="w-3.5 h-3.5" /> Player Scoring{' '}
              {isLocked && <Eye className="w-3.5 h-3.5 text-white/30" />}
            </p>
