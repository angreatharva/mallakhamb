...
export const ResponsiveTable = ({
  ...
  const defaultMobileCardRenderer = (item, index) => (
    <div
      ...
      onMouseEnter={(e) => {
        if (onRowClick) {
          e.currentTarget.style.borderColor = `${C.saffron}44`;
        ...
        return (
          <div
            ...
            style={{
              ...
              marginBottom: ci < columns.length - 1 ? 10 : 0,
            }}
          >
            <span style={{ fontSize: 12, color: textSecondary, flexShrink: 0, marginRight: 12 }}>
              {col.header}
            </span>
            ...
            {data.length === 0 ? (
              ...
              data.map((item, ri) => (
                <tr
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = `${C.saffron}0A`;
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background =
                      ri % 2 === 0 ? surface : 'rgba(255,255,255,0.015)';
                  }}
                >
                  {columns.map((col, ci) => (
                    <td
...
export const ResponsiveScoringTable = ({
  ...
  const calculateAverageMarks = (ps) => {
    ...
    if (judgeScores.length <= 3)
      return (judgeScores.reduce((a, b) => a + b, 0) / judgeScores.length).toFixed(2);
  ...
  const calculateFinalScore = (ps) => {
    ...
    return Math.max(
      0,
      avg - (parseFloat(ps.deduction) || 0) - (parseFloat(ps.otherDeduction) || 0)
    ).toFixed(2);
  };
  ...
  const ScoreInput = ({ playerId, field, placeholder, type = 'number', step, min, max }) => (
    <input
      ...
      onFocus={(e) => {
        if (!isLocked) e.target.style.borderColor = `${C.saffron}80`;
      }}
      onBlur={(e) => {
        e.target.style.borderColor = isLocked ? border : borderMid;
      }}
    />
  );
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {players.map((player) => (
            ...
            >
              {/* Header */}
              <div
                style={{ borderBottom: `1px solid ${border}`, paddingBottom: 12, marginBottom: 14 }}
              >
            ...
            {players.map((player, ri) => (
              ...
              >
                <td style={{ padding: '12px 14px' }}>
                  ...
                  {player.teamName && (
                    <p style={{ fontSize: 11, color: textSecondary }}>{player.teamName}</p>
                  )}
                </td>
                <td style={{ padding: '12px 14px' }}>
                  <ScoreInput playerId={player.id} field="time" placeholder="00:00" type="text" />
      ...
      >
        ...
        <div style={{ textAlign: 'right', flexShrink: 0, width: 'auto' }}>
          ...
          >
            ...
            <p style={{ fontSize: 22, fontWeight: 800, color: C.purple }}>
              {team.players?.length || 0}
            </p>
          </div>
          <span style={{ fontSize: 12, color: C.purpleLight, fontWeight: 500 }}>
            View Details →
          </span>
        </div>
      </div>
    ...
    <ResponsiveContainer className={className} {...props}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 10 : 14 }}>
        {teams.length === 0 ? (
          ...
          teams.map((team) => <TeamCard key={team._id} team={team} compact={isMobile} />)
        )}
      </div>
    </ResponsiveContainer>
  );
