...
export const ResponsiveForm = ({
  ...
  return (
    <ResponsiveFormContainer>
      <form onSubmit={onSubmit} className={`${getSpacingClasses()} ${className}`} {...props}>
        {children}
      </form>
    </ResponsiveFormContainer>
  );
...
export const ResponsiveFormField = ({
  ...
  return (
    <div className={`responsive-form-field ${className}`}>
      ...
      {children}
      {error && <p className={`mt-2 text-sm text-red-600 ${errorClassName}`}>{error}</p>}
    </div>
  );
};
...
export const ResponsiveInput = React.forwardRef(
  ({ type = 'text', className = '', error = false, autoComplete, ...props }, ref) => {
    const { isMobile } = useResponsive();
