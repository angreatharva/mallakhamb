  ...
  const PlayerCard = ({ player, index }) => {
    ...
    return (
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {tied && (
                <p style={{ fontSize: 11, color: C.gold, marginTop: 2 }}>{player.tieBreakNotes}</p>
              )}
            </div>
          </div>
        ...
        >
          <ScoreChip
            value={player.averageMarks?.toFixed(2) || '0.00'}
            color={C.blue}
            label="Average"
          />
        ...
        {open && player.judgeScores && (
          <div style={{ paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {['seniorJudge', 'judge1', 'judge2', 'judge3', 'judge4'].map(
              (key) =>
                player.judgeScores[key] !== undefined && (
                  <div key={key} style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span
                      style={{ fontSize: 12, color: textSecondary, textTransform: 'capitalize' }}
                    >
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: elevated }}>
              {['Rank', 'Player', 'Team', 'Time', 'Average', 'Senior Judge', 'Final Score'].map(
                (h, i) => (
                  <th
                    key={i}
                    style={{
            ...
            {players.map((player, index) => {
              ...
              return (
                <tr
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = `${C.saffron}08`;
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = ms.bg;
                  }}
                >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span
                        style={{
                          fontSize: 13,
                          fontWeight: 700,
                          color: medal ? medal.color : textSecondary,
                        }}
                      >
                        #{searchTerm ? player.originalRank : player.rank || index + 1}
                      </span>
                  ...
                  <td style={{ padding: '13px 16px' }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: textPrimary }}>
                      {player.playerName}
                    </p>
                    {tied && (
                      <p style={{ fontSize: 11, color: C.gold, marginTop: 2 }}>
                        {player.tieBreakNotes}
                      </p>
                    )}
                  </td>
                  <td
                  ...
                  <td style={{ padding: '13px 16px', whiteSpace: 'nowrap' }}>
                    ...
                    {player.baseScoreApplied && (
                      <p style={{ fontSize: 10, color: C.purple, marginTop: 3, fontWeight: 600 }}>
                        Base Score
                      </p>
                    )}
                  </td>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: isMobile ? 15 : 17, fontWeight: 700, color: textPrimary }}>
                {team.teamName}
              </p>
          ...
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            <p style={{ fontSize: isMobile ? 22 : 26, fontWeight: 800, color: C.green }}>
              {team.teamTotalScore?.toFixed(2) || '0.00'}
            </p>
            <p style={{ fontSize: 10, color: textMuted, letterSpacing: '0.04em' }}>TOTAL</p>
            <p style={{ fontSize: 11, color: textSecondary, marginTop: 2 }}>
            ...
            {team.topPlayerScores?.map((player) => (
              ...
              >
                <div>
                  ...
                  <p style={{ fontSize: 11, color: textSecondary, marginTop: 1 }}>
    ...
    <ResponsiveContainer className={className} {...props}>
      ...
      >
        ...
        <select
          ...
          onFocus={(e) => {
            if (selectedGender) e.target.style.borderColor = `${C.saffron}70`;
          }}
          onBlur={(e) => {
            e.target.style.borderColor = borderMid;
          }}
